...
SRay GenerateRay(SCamera uCamera) {
    ...
    vec3 direction = uCamera.View + uCamera.Side * coords.x * aspect + uCamera.Up * coords.y;
...
void initializeDefaultLightMaterials(out SLight light, out SMaterial materials[6]) {
    ...
    materials[1].LightCoeffs = vec4(0.4, 0.9, 0.9, 512.0);
    materials[1].ReflectionCoef = 0.8;
    materials[1].RefractionCoef = 1.0;
    materials[1].MaterialType = MIRROR_REFLECTION;
    
    // Refractive sphere (red)
    materials[2].Color = vec3(0.8, 0.0, 0.0);
    materials[2].LightCoeffs = vec4(0.4, 0.9, 0.9, 512.0);
    ...
    materials[3].LightCoeffs = vec4(0.4, 0.9, 0.0, 512.0);
    ...
    // Right wall (green)
    materials[5].Color = vec3(0.0, 1.0, 0.0);
    ...
    while(!isEmpty()) {
        ...
        if (Raytrace(ray, start, final, intersect)) {
            switch(intersect.MaterialType) {
                ...
                case REFRACTION: {
                    vec3 refractionDirection = refract(ray.Direction, intersect.Normal, 
                                                     1.0 / intersect.RefractionCoef);
